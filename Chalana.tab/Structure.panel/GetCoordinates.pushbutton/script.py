...
class CoordinateOptionsForm(WPFWindow):
    def __init__(self, xaml_file_name):
        ...
        # Force Colors Programmatically
        pure_black = Brushes.Black
        white_brush = Brushes.White
        red_accent = SolidColorBrush(Color.FromRgb(128, 47, 45))
    ...
    try:
        ...
        for p_name in param_names:
            ...
            if not p_def:
                ...
                except AttributeError: opt = DB.ExternalDefinitionCreationOptions(p_name, DB.ParameterType.Text)
        ...
        for p_def in param_defs:
            ...
            if not binding:
                ...
                if selected_founds and not bound_categories.Contains(cat_foundation):
                    bound_categories.Insert(cat_foundation); needs_rebind = True
                if selected_cols and not bound_categories.Contains(cat_column):
                    bound_categories.Insert(cat_column); needs_rebind = True
                if needs_rebind:
        ...
        if selected_founds:
            ...
            for f in founds:
                ...
                if f_type:
                    fam_name = f_type.FamilyName if hasattr(f_type, 'FamilyName') else f_type.Family.Name
                    type_name = f_type.Name if hasattr(f_type, 'Name') else f_type.get_Parameter(DB.BuiltInParameter.SYMBOL_NAME_PARAM).AsString()
                    if (fam_name + " - " + type_name) in selected_founds: all_elements.append(f)
        ...
        if selected_cols:
            ...
            for c in cols:
                ...
                if c_type:
                    fam_name = c_type.FamilyName if hasattr(c_type, 'FamilyName') else c_type.Family.Name
                    type_name = c_type.Name if hasattr(c_type, 'Name') else c_type.get_Parameter(DB.BuiltInParameter.SYMBOL_NAME_PARAM).AsString()
                    if (fam_name + " - " + type_name) in selected_cols: all_elements.append(c)
        ...
        for el in all_elements:
            pt = None
            loc = el.Location
            if loc and isinstance(loc, DB.LocationPoint): pt = loc.Point
            elif loc and isinstance(loc, DB.LocationCurve): pt = loc.Curve.Evaluate(0.5, True)
            if not pt:
                bbox = el.get_BoundingBox(None)
                if bbox: pt = (bbox.Min + bbox.Max) / 2.0
                
            if pt:
                pp = doc.ActiveProjectLocation.GetProjectPosition(pt)
                sp_x = pp.EastWest; sp_y = pp.NorthSouth; sp_z = pp.Elevation
                if not is_survey_point:
                    sp_x -= project_position.EastWest; sp_y -= project_position.NorthSouth; sp_z -= project_position.Elevation
                
                final_x = sp_x * unit_mult; final_y = sp_y * unit_mult; final_z = sp_z * unit_mult
                ...
                def set_param(element, p_name, p_val, str_val):
                    ...
                    for p in p_list:
                        if not p.IsReadOnly:
                            try:
                                if p.StorageType == DB.StorageType.String: p.Set(str_val); return True
                                else: p.Set(p_val); return True
                            except: pass
                    return False
                
                sx = "{:.3f}".format(final_x); sy = "{:.3f}".format(final_y); sz = "{:.3f}".format(final_z)
                if set_param(el, "Coord_X", final_x, sx) and set_param(el, "Coord_Y", final_y, sy) and set_param(el, "Coord_Z", final_z, sz):
                    count += 1
                
        t.Commit()
        show_custom_alert("Successfully retrieved coordinates for {} elements.".format(count))
